...
def get_Q_network(config):
    ...
    for i, n_units in enumerate(config["fcnet_hiddens"]):
        ...
        x = Dense(n_units, activation=config["fcnet_activation"], name=layer_name)(x)
    q_estimate_output = Dense(config["n_actions"], activation='softmax', name='Q_output')(x)
    # Q Model
    Q_model = Model(inputs=obs_input, outputs=q_estimate_output)
    Q_model.summary()
    Q_model.compile(optimizer=Adam(learning_rate=0.0001, clipnorm=1.0), loss='mse')
    return Q_model
...
def get_trainable_model(config):
    ...
    mask_input = Input(shape=(config["n_actions"],), name='Q_mask')
    sampled_bellman_input = Input(shape=(1,), name='Q_sampled')
